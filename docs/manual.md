...
# ── 追踪器配置 ──
tracker:
  kind: linear                    # 追踪器类型: "linear" 或 "memory"(测试用)
  endpoint: "https://api.linear.app/graphql"  # Linear API 端点 (可选)
  api_key: $LINEAR_API_KEY        # API 密钥，支持 $ENV_VAR 环境变量
  project_slug: "your-project"    # Linear 项目 slug (必填)
...
3. **理解沙箱模式**：`thread_sandbox: workspace-write` 限制 Codex 只能在工作区写文件
...
| `req` | HTTP 客户端，调用 Linear GraphQL API |
