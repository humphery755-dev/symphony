  ...
  defp format_project_link_lines do
    project_part =
      case Config.settings!().tracker do
        %{kind: "gitlab", endpoint: endpoint, project_slug: slug}
        when is_binary(slug) and slug != "" ->
          base = if is_binary(endpoint), do: String.trim_trailing(endpoint, "/"), else: "https://gitlab.com"
          colorize("#{base}/#{slug}/-/issues", @ansi_cyan)

        %{project_slug: slug} when is_binary(slug) and slug != "" ->
          colorize("https://linear.app/project/#{slug}/issues", @ansi_cyan)
